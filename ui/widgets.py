...
class HistoryItem(ctk.CTkFrame):
    def __init__(self, parent, entry, callbacks):
        super().__init__(parent, fg_color=("white", "#2b2b2b"))
        self.entry = entry
    ...
    def create_widgets(self):
        # Info
        ctk.CTkLabel(self, text=self.entry.get('title', 'Unknown'), font=ctk.CTkFont(weight="bold"), text_color=("gray10", "gray90")).pack(anchor="w", padx=10, pady=(5, 0))
        ctk.CTkLabel(self, text=f"{self.entry.get('media_type', 'Unknown')} | {self.entry.get('date')}", font=ctk.CTkFont(size=10), text_color=("gray40", "gray60")).pack(anchor="w", padx=10, pady=(0, 2))
